...
-module(internetdata_oauth_tests).
...
each_operation_requests_its_own_method_and_path_test_() ->
    {timeout, 60, fun() ->
        ...
        [begin
             ...
             [Seen] = internetdata_oauth_origin:requests(Origin),
...
every_form_leaves_encoded_exactly_as_the_corpus_says_test_() ->
    {timeout, 60, fun() ->
        ...
        [begin
             ...
             [#{headers := Headers, body := Body} = Seen] = internetdata_oauth_origin:requests(Origin),
...
every_corpus_response_decodes_with_absent_left_absent_test_() ->
    {timeout, 60, fun() ->
        ...
        Calls = [
            {<<"metadata">>, fun(Client) -> internetdata:oauth_metadata(Client) end},
            {<<"deviceAuthorization">>,
             fun(Client) -> internetdata:oauth_device_authorization(Client, ?CLIENT_ID) end},
            {<<"token">>,
             fun(Client) -> internetdata:oauth_exchange_device_code(Client, ?CLIENT_ID, <<"mo_dc_x">>) end}
        ],
        [begin
    ...
    {timeout, 60, fun() ->
        [begin
             ...
             ?assertEqual({Name, ok}, {Name, bounded(fun() ->
                 internetdata:oauth_revoke(Client, ?CLIENT_ID, <<"mo_rt_x">>)
             end)}),
             internetdata_oauth_origin:stop(Origin)
    ...
    {timeout, 60, fun() ->
        [begin
             ...
             {error, Error} = bounded(fun() ->
                 internetdata:oauth_exchange_device_code(Client, ?CLIENT_ID, <<"mo_dc_x">>)
             end),
...
only_the_idempotent_operations_retry_test_() ->
    {timeout, 60, fun() ->
        [begin
             Origin = internetdata_oauth_origin:start(Responses),
             Client = internetdata:new(#{base_url => internetdata_oauth_origin:base_url(Origin),
                                         timeout_ms => 2000}),
             Outcome = bounded(fun() -> call(Client, Operation, Args) end),
             ?assertEqual({Name, maps:get(<<"requests">>, Expect)},
                          {Name, length(internetdata_oauth_origin:requests(Origin))}),
...
poll_device_token_follows_every_corpus_case_test_() ->
    {timeout, 120, fun() ->
        [begin
             ...
             Outcome = bounded(fun() ->
                 internetdata:oauth_poll_device_token(Client, ClientId, device(Device), #{clock => Fake})
             end),
             Seen = internetdata_oauth_origin:requests(Origin),
...
no_oauth_request_carries_the_api_key_test_() ->
    {timeout, 60, fun() ->
        ...
        Origin = internetdata_oauth_origin:start(Script),
        Client = internetdata:new(#{api_key => Key, base_url => internetdata_oauth_origin:base_url(Origin),
                                    timeout_ms => 2000}),
        {_Clock, Fake} = fake_clock(),

        Outcome = bounded(fun() ->
            {ok, _} = internetdata:oauth_metadata(Client),
            {ok, Device} = internetdata:oauth_device_authorization(Client, ?CLIENT_ID,
                                                                  #{scope => <<"account.read">>}),
            {ok, _} = internetdata:oauth_exchange_device_code(Client, ?CLIENT_ID, <<"mo_dc_x">>),
            {ok, _} = internetdata:oauth_exchange_refresh_token(Client, ?CLIENT_ID, <<"mo_rt_x">>),
            ok = internetdata:oauth_revoke(Client, ?CLIENT_ID, <<"mo_rt_x">>),
            internetdata:oauth_poll_device_token(Client, ?CLIENT_ID, Device, #{clock => Fake})
        end),
...
a_2xx_that_is_not_its_type_is_an_ordinary_server_error_test_() ->
    {timeout, 60, fun() ->
        ...
        [begin
             ...
             ?assertEqual({Answer, 1}, {Answer, length(internetdata_oauth_origin:requests(Origin))}),
    ...
    {timeout, 60, fun() ->
        ...
        [begin
             ...
             ?assertEqual({Operation, Member, 1},
                          {Operation, Member, length(internetdata_oauth_origin:requests(Origin))}),
...
every_oauth_function_takes_a_per_call_timeout_test_() ->
    {timeout, 90, fun() ->
        Origin = internetdata_origin:start(#{delay_ms => 8000}),
        Client = internetdata:new(#{base_url => internetdata_origin:base_url(Origin),
                                    retries => 0, timeout_ms => 4000}),
        {_Clock, Fake} = fake_clock(),
        [#{<<"device">> := Device} | _] = maps:get(<<"cases">>, oauth(<<"poll">>)),
        PerCall = #{timeout_ms => 300},
        Calls = [
            {metadata, fun() -> internetdata:oauth_metadata(Client, PerCall) end},
            {device_authorization,
             fun() -> internetdata:oauth_device_authorization(Client, ?CLIENT_ID, PerCall) end},
            {exchange_device_code,
             fun() -> internetdata:oauth_exchange_device_code(Client, ?CLIENT_ID, <<"mo_dc">>, PerCall) end},
            {exchange_refresh_token,
             fun() -> internetdata:oauth_exchange_refresh_token(Client, ?CLIENT_ID, <<"mo_rt">>, PerCall) end},
            {revoke, fun() -> internetdata:oauth_revoke(Client, ?CLIENT_ID, <<"mo_rt_x">>, PerCall) end},
            {poll_device_token, fun() ->
                Options = PerCall#{clock => Fake},
                internetdata:oauth_poll_device_token(Client, ?CLIENT_ID, device(Device), Options)
            end}
        ],
        [begin
...
call(Client, <<"metadata">>, _Args) ->
    internetdata:oauth_metadata(Client);
call(Client, <<"deviceAuthorization">>, #{<<"clientId">> := ClientId} = Args) ->
    ...
    Options = maps:from_list([{Key, Value} || {Name, Key} <- Named, Value <- [maps:get(Name, Args, undefined)],
                                              Value =/= undefined]),
    internetdata:oauth_device_authorization(Client, ClientId, Options);
call(Client, <<"exchangeDeviceCode">>, #{<<"clientId">> := ClientId, <<"deviceCode">> := Code}) ->
    internetdata:oauth_exchange_device_code(Client, ClientId, Code);
call(Client, <<"exchangeRefreshToken">>, #{<<"clientId">> := ClientId, <<"refreshToken">> := Token}) ->
    internetdata:oauth_exchange_refresh_token(Client, ClientId, Token);
call(Client, <<"revoke">>, #{<<"clientId">> := ClientId, <<"token">> := Token}) ->
    case internetdata:oauth_revoke(Client, ClientId, Token) of
        ok -> {ok, revoked};
        Other -> Other
    end.
...
keyless(Script) ->
    Origin = internetdata_oauth_origin:start(Script),
    {Origin, internetdata:new(#{base_url => internetdata_oauth_origin:base_url(Origin),
                                timeout_ms => 2000})}.
...
atom(Name) ->
    _ = code:ensure_loaded(internetdata_oauth),
