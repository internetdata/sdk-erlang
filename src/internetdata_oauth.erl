%% @doc The OAuth device flow behind the `oauth_*' functions on `internetdata'.
%%
...
metadata(Client, Options) ->
    ...
    Request = internetdata_http:oauth_request(Bound, Path, undefined),
    internetdata_http:oauth(Bound, Request, maps:get(retries, Client), decoder(?METADATA)).
...
device_authorization(Client, ClientId, Extra, Options) ->
    ...
    Request = internetdata_http:oauth_request(Bound, <<"/oauth/device_authorization">>, Form),
    internetdata_http:oauth(Bound, Request, maps:get(retries, Client), decoder(?DEVICE_AUTHORIZATION)).
...
revoke(Client, ClientId, Token, Options) ->
    ...
    Request = internetdata_http:oauth_request(Bound, <<"/oauth/revoke">>, Form),
    Ignore = fun(_Status, _Body) -> {ok, ok} end,
    case internetdata_http:oauth(Bound, Request, maps:get(retries, Client), Ignore) of
        {ok, ok} -> ok;
...
poll(Client, ClientId, Code, Options, {Wait, Now, Deadline} = Clock, Interval) ->
    ...
    case Now() >= Deadline of
        true ->
            {error, internetdata_error:local_expiry()};
        false ->
...
exchange(Client, Form, Options) ->
    Bound = bound(Client, Options),
    Request = internetdata_http:oauth_request(Bound, <<"/oauth/token">>, Form),
    internetdata_http:oauth(Bound, Request, 0, decoder(?TOKEN_RESPONSE)).
